dom <- function(naam){
  if(!is_domeintabel(naam)) stop(paste("Er is geen domeintabel met de naam", naam))
...
create_dom_url <- function(naam, limit = 13000, offset = ""){
  glue::glue(
    "https://www.aquo.nl/index.php?title=Speciaal:Vragen&q=+[[Breder::{dom_guid(naam)}]]",
    "%0D%0A&po=?Id%0D%0A?Omschrijving%0D%0A?Begin+geldigheid%0D%0A?Eind+geldigheid%0D%0A?Gerelateerd%0D%0A",
    "&p[format]=csv&p[sep]=;&p[limit]={limit}&p[offset]={offset}"
  )
}

as.character(create_dom_url("MonsterType")) == URLencode(url_dec)
